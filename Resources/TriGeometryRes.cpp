...
unsigned TriGeometryRes::s_currentEnlightenGeometryGuid = 1;
...
TriGeometryRes::TriGeometryRes(IRoot* lockobj) :
	...
	m_inMemoryInfo( NULL ),
	m_geometryPacked( false ),
	m_immutable( false ),
	m_computeAccess( false )
{
	m_enlightenGeometryHash.m_hash1 = 0;
	m_enlightenGeometryHash.m_hash2 = 0;
...
#if INTERIORS_ENABLED
	if( gi->ExtendedData.Object )
	{
		CCP_STATS_ZONE( __FUNCTION__ " Enlighten data" );

		TriPackedGeometryData* p = (TriPackedGeometryData*)gi->ExtendedData.Object;
		if( p->GetVersion() == TriPackedGeometryData::s_versionNumber )
		{
			// Get the hashes of when the geometry was built
			m_enlightenGeometryHash.m_hash1 = p->m_geometryBuildHash1;
			m_enlightenGeometryHash.m_hash2 = p->m_geometryBuildHash2;

#if defined(ENLIGHTEN_PRECOMPUTE_ENABLED)
			m_packedGeometry.Delete();
			m_packedGeometry = Geo::GeoAutoReleasePtr< Enlighten::IPrecompPackedGeometry>( Enlighten::IPrecompPackedGeometry::Create() );

			GeoMemoryStream geoStream( p->m_bufferSize, p->m_buffer );
			unsigned int version;
			geoStream.Read( &version, sizeof( version ), 1 );
			m_packedGeometry->Load( geoStream );
#endif
			m_geometryPacked = true;
		}
	}
#endif
...
void AddVerticesToEnlighten( void* context, const Vector3& p1, const Vector3& n1, const Vector2& uv1 )
{
	Geo::AtlasMeshVertices* p = (Geo::AtlasMeshVertices*)context;
	Geo::AtlasMeshVertex vert = { {p1.x, p1.y, p1.z}, {n1.x, n1.y, n1.z}, {uv1.x, uv1.y} };
	p->Push( vert );
}
...
// ---------------------------------------------------------------------------------------
// Description:
//   Creates Enlighten packed geometry.
// Arguments:
//   pPrecompute - Enlighten precompute object.
//   outputPixelSize - the size of each enlighten texel in meters. Must be consistent
//                     with the system that will use it.
// Return Value:
//   Enlighten packed geometry or NULL if there was an error creating it
// ---------------------------------------------------------------------------------------
Enlighten::IPrecompPackedGeometry* TriGeometryRes::GetEnlightenPackedGeometry( Enlighten::IPrecompute* pPrecompute,
																			   TriEnlightenProgressBar& prog,
																			   float outputPixelSize /*= 1.0f */ )
{
	// If we've already generated packed geometry in one system, don't do it in another
	if( m_packedGeometry && !g_geometryResPackedGeometryRebuild )
	{
	...
	if( !IsGood() )
	{
		CCP_LOGWARN( "Mesh is not prepared yet." );
		return NULL;
	}

	if( m_geometryPacked && !g_geometryResPackedGeometryRebuild )
	{
		return m_packedGeometry.GetPtr();
	}
	...
	if ( m_packedGeometry )
	{
		CCP_LOG( "Recreating precompute input geometry with id: %i, %i",1,m_packedGeometry->GetId().Low());
		geom = Enlighten::IPrecompInputGeometry::Create( m_packedGeometry->GetId() );
	}
	else
	{
		CCP_LOG( "Creating new precompute input geometry with id: %i, %i",1,s_currentEnlightenGeometryGuid);
		geom = Enlighten::IPrecompInputGeometry::Create( Geo::GeoGuid::Create( 1, s_currentEnlightenGeometryGuid++ ) );
	}


	// Required for making new vertex buffers
	// This bit of the code will eventually need to be moved
	CCP_LOG( "Creating UV Charts" );
	prog.SetProgressString( "Creating UV Charts" );

	for( unsigned meshIx = 0; meshIx < m_meshes.size(); ++meshIx )
	{
		Enlighten::PrecompMeshProperties precomputeMeshProperties;

		Enlighten::IPrecompInputMesh* precomputeMesh = CreateEnlightenInputMesh( meshIx, false );
	...
	for( unsigned meshIx = 0; meshIx < m_meshes.size(); ++meshIx )
	{
		TriGeometryResMeshData* currentMesh = m_meshes[meshIx];
		if( currentMesh == NULL )
		{
			continue;
		}
		...
		const unsigned bytesPerVertex = currentMesh->m_bytesPerVertex;

		uint8_t* originalVBufferContents = NULL;
		currentMesh->m_vertexBuffer.Lock(0,0, (void**)&originalVBufferContents, LOCK_READONLY, renderContext );
		ON_BLOCK_EXIT( [&]{ currentMesh->m_vertexBuffer.Unlock( renderContext ); } );

		Tr2VertexDefinition decl;
		if ( !Tr2EffectStateManager::GetVertexDeclarationElements( currentMesh->m_vertexDeclaration, decl ) )
		{
			return false;
		}

		uint16_t texCoord1Offset = 0;
		
		bool isHalfPrecision = false;

		for( size_t j = 0; j != decl.m_items.size(); j++ )
		{
			if ( decl.m_items[j].m_usage == Tr2VertexDefinition::TEXCOORD && decl.m_items[j].m_usageIndex == 1 )
			{
				texCoord1Offset = decl.m_items[j].m_offset;
				if( decl.m_items[j].m_dataType == Tr2VertexDefinition::FLOAT16_2 )
				{
		...
		if( isHalfPrecision )
		{
			for( unsigned i = 0; i < currentMesh->m_vertexCount; ++i )
			{
				D3DXFloat32To16Array( ((D3DXFLOAT16*)(originalVBufferContents +(i*bytesPerVertex) + texCoord1Offset)),
					&(outputUVs[i].U), 2 );
			}
		}
		else
		{
			for( unsigned i = 0; i < currentMesh->m_vertexCount; ++i )
			{
				float* const uvPtr = (float*)((uint8_t*)originalVBufferContents +(i*bytesPerVertex) + texCoord1Offset);
				*uvPtr = outputUVs[i].U;
	...
	CCP_LOG("Done processing Mesh for enlighten.");
	// Don't release the packed geometry, allow it to go up
	//packedGeometryReleaseGuard.Dismiss();
	m_packedGeometry.Delete();
...
// ---------------------------------------------------------------------------------------
// Description:
//   Performs Enlighten deferred projection. Projects this geometry onto the given target
//   geometry.
// Arguments:
//   pPrecompute - Enlighten precompute object.
//   deferredTarget - Geometry resource containing target geometry.
//   outputPixelSize - the size of each enlighten texel in meters. Must be consistent
//                     with the system that will use it.
// ---------------------------------------------------------------------------------------
void TriGeometryRes::ProjectEnlightenGeometry( Enlighten::IPrecompute* pPrecompute,
											   TriGeometryRes* deferredTarget,
											   TriEnlightenProgressBar& prog,
											   float outputPixelSize /*= 1.0f */ )
{
	// If we've already generated packed geometry in one system, don't do it in another
	...
	if( !deferredTarget->IsGood() )
	{
		CCP_LOGWARN( "Target mesh is not prepared yet." );
		return;
	}

	if( deferredTarget->m_meshes.empty() )
	{
		CCP_LOGWARN( "Target mesh does not have meshes." );
		return;
	}
	...
	// Add target mesh from deferredTarget geometry
	{
		Enlighten::IPrecompInputMesh* precomputeMesh = deferredTarget->CreateEnlightenInputMesh( 0, true );
...
// ---------------------------------------------------------------------------------------
// Description:
//   Creates Enlighten input geometry for a mesh to be used in geometry packing or
//   deferred projection.
// Arguments:
//   meshIx - Index of the mesh in m_meshes array to create input geometry for.
...
// Return value:
//   New Enlighten input mesh or NULL if the process failed.
// ---------------------------------------------------------------------------------------
Enlighten::IPrecompInputMesh* TriGeometryRes::CreateEnlightenInputMesh( int meshIx, bool copyChartUVFromPackedGeometry )
{
	USE_MAIN_THREAD_RENDER_CONTEXT();
	...
	Geo::AtlasMeshVertices verts( currentMesh->m_vertexCount );
	Geo::AtlasMeshTriangles triangles( currentMesh->m_primitiveCount );

	ProcessMeshVerticesWithUV( meshIx, &AddVerticesToEnlighten, &verts );

	int bytesPerVertex = currentMesh->m_bytesPerVertex;

	void* pSrcIndices = NULL;
	CR_RETURN_VAL( currentMesh->m_indexBuffer.Lock( 0, 0, &pSrcIndices, LOCK_READONLY, renderContext ), NULL );

	unsigned short* pShortIndices = (unsigned short*)pSrcIndices;
	unsigned long* pLongIndices = (unsigned long*)pSrcIndices;

	for( unsigned i = 0; i < currentMesh->m_primitiveCount; ++i )
	{
		if( currentMesh->m_indexBuffer.Is16Bit() )
		{
			Geo::AtlasMeshTriangle triangle = {{*pShortIndices, *(pShortIndices+1), *(pShortIndices+2)}, 0};
			triangles.Push( triangle );
			pShortIndices += 3;
		}
		else
		{
			Geo::AtlasMeshTriangle triangle = {{*pLongIndices, *(pLongIndices+1), *(pLongIndices+2)}, 0};
			triangles.Push( triangle );
			pLongIndices += 3;
		}
	}
	CR( currentMesh->m_indexBuffer.Unlock( renderContext ) );
	...
	// ======================================================
	// Add another UV set after the first one.
	// ======================================================
	if ( !m_packedGeometry )
	{
		Tr2VertexDefinition original;
		Tr2VertexDefinition extraUV;

		if ( !Tr2EffectStateManager::GetVertexDeclarationElements( currentMesh->m_vertexDeclaration, original ) )
		{
			return nullptr;
		}
		...
		auto texcoord = extraUV.Find( extraUV.TEXCOORD );
		if( !texcoord )
		{
			return nullptr;
		}

		const unsigned texcoordByteSize = extraUV.GetDataTypeSizeInBytes( texcoord->m_dataType );

		for( size_t i = 0; i != extraUV.m_items.size(); ++i )
		{
			if( extraUV.m_items[i].m_offset > texcoord->m_offset )
			{
				extraUV.m_items[i].m_offset += texcoordByteSize;
			}
		}

		Tr2VertexDefinition::Item newItem = *texcoord;
		newItem.m_offset += texcoordByteSize;
		newItem.m_usageIndex = 1;

		extraUV.m_items.push_back( newItem );
		extraUV.m_nextOffset[0] += texcoordByteSize;

		std::sort( begin( extraUV.m_items ), end( extraUV.m_items ) );


		// Now we need to change the vertex buffer
		// We can use Granny to make this simpler			

		granny_data_type_definition oldVertexDecl[MAXD3DDECLLENGTH];
		granny_data_type_definition newVertexDecl[MAXD3DDECLLENGTH];
		bool success = ConvertVertexDeclToGranny( original, oldVertexDecl, MAXD3DDECLLENGTH ) &&
			ConvertVertexDeclToGranny( extraUV , newVertexDecl, MAXD3DDECLLENGTH );
		...
		Tr2VertexBufferAL	newVBuffer;
		{
			USE_MAIN_THREAD_RENDER_CONTEXT();
			CR_RETURN_VAL(	
				newVBuffer.Create( currentMesh->m_vertexCount * bytesPerVertex, renderContext )
				, nullptr );
		}

		void* newVBufferContents = NULL;
		CR_RETURN_VAL( newVBuffer.Lock(0,0, &newVBufferContents, LOCK_WRITEONLY, renderContext ), nullptr );

		void* originalVBufferContents = NULL;
		CR_RETURN_VAL( currentMesh->m_vertexBuffer.Lock(0,0, &originalVBufferContents, LOCK_READONLY, renderContext ), nullptr );


		GrannyConvertVertexLayouts(
			currentMesh->m_vertexCount,
			oldVertexDecl,
			originalVBufferContents,
			newVertexDecl,
			newVBufferContents
			);

		newVBuffer.Unlock( renderContext );
		currentMesh->m_vertexBuffer.Unlock( renderContext );
		currentMesh->m_vertexBuffer = std::move( newVBuffer );

		currentMesh->m_vertexDeclaration = newVertDecl;
		currentMesh->m_bytesPerVertex = extraUV.m_nextOffset[0];
	}
	...
	//////////////////////////////////////////////////////////////////////////
	const unsigned refactoredVBVertexCount = currentMesh->m_vertexCount + refactorSize;
	const unsigned originalVBVertexCount = currentMesh->m_vertexCount;
	currentMesh->m_vertexCount = refactoredVBVertexCount;
	...
	uint16_t texCoord0Offset = 0;
	uint16_t texCoord1Offset = 0;
	bool isHalfPrecision0 = false;
	bool isHalfPrecision1 = false;

	for ( size_t j = 0; j != decl.m_items.size(); ++j )
	{
		if ( decl.m_items[j].m_usage == Tr2VertexDefinition::TEXCOORD && decl.m_items[j].m_usageIndex == 0 )
		{
			texCoord0Offset = decl.m_items[j].m_offset;
			if( decl.m_items[j].m_dataType == Tr2VertexDefinition::FLOAT16_2 )
			{
				isHalfPrecision0 = true;
			}
		}

		if ( copyChartUVFromPackedGeometry && decl.m_items[j].m_usage == Tr2VertexDefinition::TEXCOORD && decl.m_items[j].m_usageIndex == 1 )
		{
			texCoord1Offset = decl.m_items[j].m_offset;
			if( decl.m_items[j].m_dataType == Tr2VertexDefinition::FLOAT16_2 )
			{
				isHalfPrecision1 = true;
			}
		}
	}
	...
	if( refactorSize > 0 )
	{
		...
		// same decl and therefore size, we're going to replace the original UVs
		{
			CR_RETURN_VAL( 
					newVBuffer.Create( refactoredVBVertexCount * bytesPerVertex, renderContext )
					, nullptr );
		}

		void* newVBufferContents = NULL;
		newVBuffer.Lock(0,0, &newVBufferContents, LOCK_WRITEONLY, renderContext );

		void* originalVBufferContents = NULL;
		currentMesh->m_vertexBuffer.Lock(0,0, &originalVBufferContents, LOCK_READONLY, renderContext );

		// Copy the original contents
		memcpy_s(
			newVBufferContents,
			refactoredVBVertexCount * currentMesh->m_bytesPerVertex,
			originalVBufferContents,
			originalVBVertexCount * bytesPerVertex );

		CR_RETURN_VAL( currentMesh->m_indexBuffer.Lock( 0, 0, &pSrcIndices, LOCK_READONLY, renderContext ), false );
		pShortIndices = (unsigned short*)pSrcIndices;
		pLongIndices = (unsigned long*)pSrcIndices;
		...
		for( Geo::AtlasMeshVertexSplitResult *i = flattenAndPackResults->m_VertexSplittings.Begin(); i != flattenAndPackResults->m_VertexSplittings.End(); ++i )
		{
			...
			// Copy the vertex
			memcpy_s( (uint8_t*)newVBufferContents +(newVertexID*bytesPerVertex), bytesPerVertex,  (uint8_t*)originalVBufferContents + (oldVertexID*bytesPerVertex), bytesPerVertex );
			...
			for( const Geo::AtlasMeshTriangleCorner* x = duplicationVtx.m_CornersToRelink.Begin(); x != duplicationVtx.m_CornersToRelink.End(); ++x)
			{
				...
				if( currentMesh->m_indexBuffer.Is16Bit() )
				{
					CCP_ASSERT( pShortIndices[(corner.m_TriangleIndex*3) + corner.m_TriangleCorner] == oldVertexID );
					pShortIndices[(corner.m_TriangleIndex*3) + corner.m_TriangleCorner] = newVertexID;
				}
				else
				{
					CCP_ASSERT( pLongIndices[(corner.m_TriangleIndex*3) + corner.m_TriangleCorner] == oldVertexID );
					pLongIndices[(corner.m_TriangleIndex*3) + corner.m_TriangleCorner] = newVertexID;
				}
			}
		}

		currentMesh->m_indexBuffer.Unlock( renderContext );
		newVBuffer.Unlock( renderContext );
		currentMesh->m_vertexBuffer = std::move( newVBuffer );
	}
	...
	uint8_t* vBuffPtr = NULL;
	CR_RETURN_VAL( currentMesh->m_vertexBuffer.Lock(0,0, (void**)&vBuffPtr, LOCK_READONLY, renderContext ), nullptr );

	for( int i = 0; i < finalAtlasMesh->GetVertexCount(); ++i )
	{
		Geo::AtlasMeshVertex const * const vert = finalAtlasMesh->GetVertex(i);
		// Grab the original diffuse map texture coordinates
		float uv0[2] = { 0.0f, 0.0f };
		if( isHalfPrecision0 )
		{
			D3DXFloat16To32Array( (float*)&uv0, (const D3DXFLOAT16*)(vBuffPtr + texCoord0Offset + i*bytesPerVertex ), 2 );
		}
		else
		{
			uv0[0] = ((float*)( vBuffPtr + i*bytesPerVertex + texCoord0Offset ) )[0];
			uv0[1] = ((float*)( vBuffPtr + i*bytesPerVertex + texCoord0Offset ) )[1];
		}
		...
		if( copyChartUVFromPackedGeometry )
		{
			if( isHalfPrecision1 )
			{
				D3DXFloat16To32Array( (float*)&uv1, (const D3DXFLOAT16*)(vBuffPtr + texCoord1Offset + i*bytesPerVertex ), 2 );
			}
			else
			{
				uv1[0] = ((float*)( vBuffPtr + i*bytesPerVertex + texCoord1Offset ) )[0];
				uv1[1] = ((float*)( vBuffPtr + i*bytesPerVertex + texCoord1Offset ) )[1];
			}
		}
		else
		{
		...
		const Enlighten::PrecompInputVertex newVert = {
			...
			Geo::GeoPoint2( *uv1, *(uv1+1) ),
			// AlbedoUV = diffuse texture coordinates
			Geo::GeoPoint2( *uv0, *(uv0+1) )
		};
	...
	for( int i = 0; i < finalAtlasMesh->GetTriangleCount(); ++i )
	{
		...
		int materialID = 0;
		for( unsigned int areaID = 0; areaID < currentMesh->m_areas.size(); ++areaID )
		{
			TriGeometryResAreaData& a = currentMesh->m_areas[areaID];
			if( (a.m_firstIndex/3) <= i && (a.m_firstIndex/3)+a.m_primitiveCount > i )
			{
				materialID = areaID;
				break;
			}
...
void TriGeometryRes::GetEnlightenGeometryHashes( unsigned int& hash1, unsigned int& hash2 ) const
{
	hash1 = m_enlightenGeometryHash.m_hash1;
	hash2 = m_enlightenGeometryHash.m_hash2;
}
...
bool TriGeometryRes::CreateMeshFromGrannyMesh( granny_mesh* myMesh, TriGeometryResMeshData* pMesh, Tr2PrimaryRenderContext& renderContext, void* pVBOverride )
{
	...
	if( myMesh->Name && strncmp( myMesh->Name, "gpuraw_", 7 ) == 0 )
	{
		...
		if( forceFullFloat )
		{
			...
			USE_MAIN_THREAD_RENDER_CONTEXT();
			CR_RETURN_VAL( pMesh->m_shaderResourceBuffer.CreateStructured( vertexCount, bytesPerVertex, usage, &tempBuffer[0], renderContext ), false );
		}
		else
		{
			USE_MAIN_THREAD_RENDER_CONTEXT();
			CR_RETURN_VAL( pMesh->m_shaderResourceBuffer.CreateStructured( vertexCount, bytesPerVertex, usage, pSrc, renderContext ), false );
		}
	...
	if( m_immutable )
	{
		...
		USE_MAIN_THREAD_RENDER_CONTEXT();
		CR_RETURN_HR( d3dIB.Create( indexCount, USAGE_CPU_READ | USAGE_IMMUTABLE, bytesPerIndex == 2 ? IB_16BIT : IB_32BIT, &tempBuffer[0], renderContext ) );
	}
	else
	{
		{
